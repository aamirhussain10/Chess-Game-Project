"""
chess.py — Chess engine (bitboard-based)
"""
...
# Build reverse mapping (str -> piece code)
PIECE_CODES.update({v: k for k, v in list(PIECE_CODES.items())})
...
class Game:
    def __init__(self, FEN=''):
        self.board = list(INITIAL_BOARD)  # BUG FIX: use list() copy, not reference
        self.to_move = WHITE
        self.ep_square = 0
...
def north_one(bitboard):
    return (bitboard << 8) & nnot(RANK_1)  # BUG FIX: was nnot(RANK_1) which is correct; keep
...
def knight_attacks(moving_piece):
    return (knight_NNE(moving_piece) | knight_ENE(moving_piece) |
            knight_NNW(moving_piece) | knight_WNW(moving_piece) |
            knight_SSE(moving_piece) | knight_ESE(moving_piece) |
            knight_SSW(moving_piece) | knight_WSW(moving_piece))
...
def get_attacks(moving_piece, board, color):
    ...
    return 0  # BUG FIX: return 0 if no piece matched (was implicit None)
...
def get_moves(moving_piece, game, color):
    ...
    return 0  # BUG FIX: return 0 if no piece matched
...
def count_attacks(target, board, attacking_color):
    ...
    for index in range(64):
        ...
        if piece != EMPTY and piece & COLOR_MASK == attacking_color:
            pos = 0b1 << index
            attacks = get_attacks(pos, board, attacking_color)
            if attacks and attacks & target:  # BUG FIX: guard None from get_attacks
                attack_count += 1
    return attack_count
...
def random_move(game, color):
    return choice(list(legal_moves(game, color)))  # BUG FIX: choice() needs a sequence, not generator
...
def evaluated_move(game, color):
    ...
    if not best_moves:  # BUG FIX: guard empty list
            ...
            if score > alpha:
                ...
                if alpha > beta:
                    if verbose:
                        print('\t' * depth + 'cutoff')
                    break
        return [choice(best_moves), alpha] if best_moves else [None, alpha]
    if color == BLACK:
        for move in legal_moves(game, color):
            if verbose:
                print('\t' * depth + str(depth) + '. evaluating ' + PIECE_CODES[get_piece(game.board, move[0])] + move2str(move))
            ...
            if score < beta:
                ...
                if alpha > beta:
                    if verbose:
                        print('\t' * depth + 'cutoff')
                    break
        return [choice(best_moves), beta] if best_moves else [None, beta]
...
def get_AI_move(game, depth=2):
    ...
    end_time = time()
    if verbose:
        print('Found move ' + PIECE_CODES[get_piece(game.board, move[0])] + ' from ' +
              str(bb2str(move[0])) + ' to ' + str(bb2str(move[1])) +
              ' in {:.3f} seconds'.format(end_time - start_time))
    return move
...
def watch_AI_game(game=None, sleep_seconds=0):
    if game is None:
        game = Game()
...
def find_in_book(game):
    """Try to find a book move. Returns list of openings or empty list."""
    ...
    try:
        ...
        return []  # BUG FIX: gracefully handle missing book.txt
